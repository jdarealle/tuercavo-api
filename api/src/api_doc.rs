...
pub fn router(mut spec: OpenApi, cookie_name: &str) -> Router<AppState> {
    ...
    spec.info.description = Some("Catálogo con permisos locales. Abre /api/auth/login en este mismo origen para iniciar sesión. El navegador envía la cookie HttpOnly automáticamente; las escrituras requieren Origin del mismo origen. PATCH conserva los campos omitidos y null limpia solo los campos anulables.".into());
    spec.components
