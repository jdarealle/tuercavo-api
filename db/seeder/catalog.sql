-- Datos opcionales para desarrollo. No es una migración.
...
INSERT INTO seed_categories (public_id, name, description, status) VALUES
    ('10000000-0000-4000-8000-000000000001', 'Herramientas manuales', 'Herramientas de uso manual para trabajos de reparación, montaje y mantenimiento.', 'active'),
    ('10000000-0000-4000-8000-000000000002', 'Herramientas eléctricas', 'Equipos eléctricos portátiles para perforación, corte, lijado y desbaste.', 'active'),
    ('10000000-0000-4000-8000-000000000003', 'Tornillería y fijación', 'Tornillos, taquetes, clavos, tuercas y otros elementos de sujeción.', 'active'),
    ('10000000-0000-4000-8000-000000000004', 'Plomería', 'Conexiones, válvulas, tubería y accesorios para instalaciones hidráulicas.', 'active'),
    ('10000000-0000-4000-8000-000000000005', 'Electricidad', 'Material eléctrico para instalaciones residenciales y comerciales.', 'active'),
    ('10000000-0000-4000-8000-000000000006', 'Pinturas y acabados', 'Pinturas, recubrimientos y accesorios para preparación y acabado de superficies.', 'active'),
    ('10000000-0000-4000-8000-000000000007', 'Construcción', 'Materiales y herramientas auxiliares para obra y remodelación.', 'active'),
    ('10000000-0000-4000-8000-000000000008', 'Seguridad industrial', 'Equipo de protección personal y señalización para el área de trabajo.', 'active'),
    ('10000000-0000-4000-8000-000000000009', 'Jardinería', 'Herramientas y accesorios para cuidado de jardines y áreas exteriores.', 'inactive'),
    ('10000000-0000-4000-8000-000000000010', 'Adhesivos y selladores', 'Soluciones para unir, rellenar y sellar diferentes materiales.', 'archived');

INSERT INTO categories (public_id, name, description, status)
SELECT public_id, name, description, status::catalog_status
FROM seed_categories
ON CONFLICT DO NOTHING;

DO $seed_check$
BEGIN
    IF (SELECT count(*) FROM categories c JOIN seed_categories s USING (public_id)) <> 10 THEN
        RAISE EXCEPTION 'No fue posible registrar las 10 categorías de desarrollo; revisa conflictos de nombre o UUID';
    END IF;
END
$seed_check$;
...
INSERT INTO seed_suppliers (public_id, code, name, contact_name, email, phone, status) VALUES
    ('20000000-0000-4000-8000-000000000001', 'FERRE-001', 'Distribuidora Ferretera del Norte', 'Mariana López', 'ventas@ferreteradelnorte.example', '+52 664 555 0101', 'active'),
    ('20000000-0000-4000-8000-000000000002', 'HERRA-002', 'Herramientas Profesionales de México', 'Carlos Ramírez', 'pedidos@herramientaspro.example', '+52 81 5555 0102', 'active'),
    ('20000000-0000-4000-8000-000000000003', 'CONST-003', 'Suministros para Construcción Baja', 'Alejandra Torres', 'contacto@construccionbaja.example', '+52 664 555 0103', 'active'),
    ('20000000-0000-4000-8000-000000000004', 'ELEC-004', 'Material Eléctrico Nacional', 'Jorge Mendoza', 'mayoreo@materialelectrico.example', '+52 55 5555 0104', 'active'),
    ('20000000-0000-4000-8000-000000000005', 'JARD-005', 'Equipos y Jardines del Pacífico', 'Sofía Navarro', 'ventas@jardinespacifico.example', '+52 33 5555 0105', 'inactive');

INSERT INTO suppliers (public_id, code, name, contact_name, email, phone, status)
SELECT public_id, code, name, contact_name, email, phone, status::catalog_status
FROM seed_suppliers
ON CONFLICT DO NOTHING;

DO $seed_check$
BEGIN
    IF (SELECT count(*) FROM suppliers p JOIN seed_suppliers s USING (public_id)) <> 5 THEN
        RAISE EXCEPTION 'No fue posible registrar los 5 proveedores de desarrollo; revisa conflictos de código o UUID';
    END IF;
END
$seed_check$;
...
INSERT INTO seed_products (
    ...
    ('30000000-0000-4000-8000-000000000011', 'TF-TOR-1X8', 'Tornillo para madera 1 x 8', 'Tornillo galvanizado de cabeza plana para madera.', 'Fixser', '10000000-0000-4000-8000-000000000003', '20000000-0000-4000-8000-000000000001', 'box', 84.50, 'active'),
    ('30000000-0000-4000-8000-000000000012', 'TF-TAQ-14', 'Taquete plástico 1/4 pulgada', 'Taquete expansivo para concreto y mampostería.', 'Anclo', '10000000-0000-4000-8000-000000000003', '20000000-0000-4000-8000-000000000003', 'pack', 42.00, 'active'),
    ...
    ('30000000-0000-4000-8000-000000000014', 'TF-TUE-14', 'Tuerca hexagonal 1/4 pulgada', 'Tuerca zincada estándar para fijación mecánica.', 'Fixser', '10000000-0000-4000-8000-000000000003', '20000000-0000-4000-8000-000000000001', 'box', 115.00, 'active'),
    ('30000000-0000-4000-8000-000000000015', 'TF-ARA-14', 'Arandela plana 1/4 pulgada', 'Arandela galvanizada para distribuir carga de apriete.', 'Fixser', '10000000-0000-4000-8000-000000000003', NULL, 'box', 89.00, 'active'),
    ...
    ('30000000-0000-4000-8000-000000000020', 'PL-CIN-TEF', 'Cinta selladora PTFE 12 mm', 'Cinta para sellar uniones roscadas en instalaciones hidráulicas.', 'Foset', '10000000-0000-4000-8000-000000000004', NULL, 'piece', 18.50, 'active'),
    ...
    ('30000000-0000-4000-8000-000000000025', 'EL-LED-9', 'Foco LED 9 W luz cálida', 'Foco ahorrador con base E27 y flujo de 800 lúmenes.', 'Philips', '10000000-0000-4000-8000-000000000005', '20000000-0000-4000-8000-000000000004', 'box', 239.00, 'active'),

    ('30000000-0000-4000-8000-000000000026', 'PA-VIN-4B', 'Pintura vinílica blanca 4 L', 'Pintura mate para interiores con buena cobertura y bajo olor.', 'Comex', '10000000-0000-4000-8000-000000000006', '20000000-0000-4000-8000-000000000001', 'liter', 139.75, 'active'),
    ...
    ('30000000-0000-4000-8000-000000000030', 'PA-LIJ-120', 'Lija para madera grano 120', 'Hoja abrasiva para preparación y acabado de madera.', 'Fandeli', '10000000-0000-4000-8000-000000000006', '20000000-0000-4000-8000-000000000002', 'pack', 95.00, 'active'),
    ...
    ('30000000-0000-4000-8000-000000000037', 'SI-GUA-N', 'Guantes de nitrilo reforzado', 'Guantes reutilizables resistentes a abrasión y aceites.', '3M', '10000000-0000-4000-8000-000000000008', '20000000-0000-4000-8000-000000000001', 'pack', 149.00, 'active'),
    ...
    ('30000000-0000-4000-8000-000000000040', 'SI-MAS-N95', 'Mascarilla respiratoria N95', 'Respirador desechable para partículas sin aceite.', '3M', '10000000-0000-4000-8000-000000000008', '20000000-0000-4000-8000-000000000001', 'box', 699.00, 'active'),
    ...
    ('30000000-0000-4000-8000-000000000047', 'AS-PEG-5', 'Pegamento epóxico 5 minutos', 'Adhesivo bicomponente para metal, madera y cerámica.', 'Resistol', '10000000-0000-4000-8000-000000000010', '20000000-0000-4000-8000-000000000001', 'pack', 149.00, 'archived'),
...
DO $seed_check$
BEGIN
    IF (SELECT count(*) FROM products p JOIN seed_products s USING (public_id)) <> 50 THEN
        RAISE EXCEPTION 'No fue posible registrar los 50 productos de desarrollo; revisa conflictos de SKU o UUID';
    END IF;
END
$seed_check$;
