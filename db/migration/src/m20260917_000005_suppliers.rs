...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager.create_table(Table::create().table(Suppliers::Table)
            ...
            .col(ColumnDef::new(Suppliers::Code).string_len(64).not_null())
            ...
            .check(("ck_suppliers_code", Expr::cust(r#"char_length(code) BETWEEN 1 AND 64 AND code ~ '^[A-Za-z0-9][A-Za-z0-9._-]*$'"#)))
            ...
            .check(("ck_suppliers_email", Expr::cust(r#"char_length(email) BETWEEN 1 AND 254 AND email = btrim(email) AND email !~ '[[:space:][:cntrl:]]'"#)))
        ...
        manager
            .create_index(
                Index::create()
                    ...
                    .col(Func::lower(Expr::col(Suppliers::Code)))
                    .unique()
                    .to_owned(),
            )
            .await?;
