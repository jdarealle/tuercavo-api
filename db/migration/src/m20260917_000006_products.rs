...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager.create_table(Table::create().table(Products::Table)
            ...
            .col(ColumnDef::new(Products::Sku).custom("citext").not_null())
            ...
            .index(Index::create().name("uq_products_sku").unique().col(Products::Sku))
            ...
            .check(("ck_products_sku", Expr::cust(r#"char_length(sku) BETWEEN 1 AND 64 AND sku::text ~ '^[A-Za-z0-9][A-Za-z0-9._-]*$'"#)))
