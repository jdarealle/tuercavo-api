...
Los usuarios nuevos comienzan sin departamento. `GET /api/departments` lista los departamentos y `GET /api/departments/{public_id}` consulta uno mediante su UUID público; ambos requieren `departments.read`. `POST /api/departments` recibe `{"name":"Ventas"}` y requiere `departments.create`. Los nombres son únicos sin distinguir mayúsculas. `PUT /api/users/{public_id}/department` recibe `{"department_public_id":"UUID"}` para asignar un departamento existente o `{"department_public_id":null}` para quitarlo; requiere `users.assign_department`. **Solo el rol local `admin` puede usar estas rutas de departamentos y asignación**; la API tampoco permite otorgar esos permisos a roles personalizados. El campo es obligatorio en el cuerpo. Las respuestas de usuarios y `/api/auth/me` incluyen `department_public_id`, que puede ser `null`. El ID entero del departamento se usa solo como FK interna. Esta asignación no cambia roles, permisos ni sesiones.
